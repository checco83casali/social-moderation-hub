...
// $violationRetentionDays è già risolto dal controller (eredita $retentionDays
// se non impostato esplicitamente).
...
<div class="lang-section" lang="it">
  ...
  <div class="test-card">
    ...
    <div class="tc-body">
      ...
      <p><strong>Limitazione della conservazione:</strong> <?php if ($retentionEnabled): ?>i dati identificativi degli utenti senza violazioni (contenuto del commento, nome, profilo) vengono anonimizzati automaticamente dopo <?= (int) $retentionDays ?> giorni di inattività, parametro configurabile dal Titolare (vedi §7 privacy policy) — il trattamento non è quindi indefinito.<?php else: ?>la finestra generale di anonimizzazione automatica non è attualmente configurata (vedi §7 privacy policy); il Titolare dovrebbe impostarla per limitare la durata del trattamento al minimo necessario, coerentemente con la conclusione di questo Necessity Test.<?php endif; ?> Per gli utenti con almeno una violazione/ban registrati si applica una finestra dedicata, indipendente da quella generale (vedi §4 privacy policy): <?php if ($violationRetentionDiffers): ?><?= (int) $violationRetentionDays ?> giorni dall'ultima violazione.<?php elseif ($violationRetentionEnabled): ?>coincide con quella generale, non essendo stata impostata separatamente.<?php else: ?>non configurata.<?php endif; ?></p>
    </div>
  </div>
...
<div class="lang-section" lang="en">
  ...
  <div class="test-card">
    ...
    <div class="tc-body">
      ...
      <p><strong>Storage limitation:</strong> <?php if ($retentionEnabled): ?>identifying data for users with no recorded violation (comment content, name, profile) is automatically anonymised after <?= (int) $retentionDays ?> days of inactivity, a parameter the Controller can configure (see §7 of the privacy policy) — processing is therefore not indefinite.<?php else: ?>the general automatic anonymisation window is not currently configured (see §7 of the privacy policy); the Controller should set one to limit the duration of processing to what is necessary, consistent with the conclusion of this Necessity Test.<?php endif; ?> Users with at least one recorded violation/ban are subject to a dedicated window, independent of the general one (see §4 of the privacy policy): <?php if ($violationRetentionDiffers): ?><?= (int) $violationRetentionDays ?> days from the last violation.<?php elseif ($violationRetentionEnabled): ?>same as the general window, since none was set separately.<?php else: ?>not configured.<?php endif; ?></p>
    </div>
  </div>
