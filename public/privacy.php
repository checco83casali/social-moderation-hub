...
<h2>11. Minori</h2>
<p>Una Pagina Facebook pubblica non consente di verificare l'età di chi commenta: la Piattaforma non richiede né riceve alcun segnale di età dall'utente. La presenza di minori tra i commentatori è quindi <strong>prevedibile</strong>, non un'eccezione che il Titolare possa escludere per policy. Per lo stesso motivo il Titolare non può promettere di riconoscere e cancellare "immediatamente" i dati di un minore al momento della raccolta: non esiste, né potrebbe esistere in modo affidabile, un meccanismo tecnico che dedurre l'età da un commento pubblico.</p>
<p>La risposta a questa prevedibilità non è la negazione, ma le stesse tutele già applicate a <em>tutti</em> gli utenti fin dal primo commento, indipendentemente dall'età: nessun ban alla prima violazione (serve un pattern di recidiva, vedi §5.4), ogni ban è sempre temporaneo, la revisione umana dei casi incerti è cieca (il moderatore non vede mai il nome reale, vedi §5.4) e ogni decisione — automatica o umana — resta sempre contestabile tramite la procedura di appello (§8).</p>
<p>Un minore, o chi ne fa le veci, può comunque richiedere in qualsiasi momento la cancellazione dei propri dati o l'esercizio di uno dei diritti di cui al §8, scrivendo a <a href="mailto:<?= $orgEmail ?>"><?= $orgEmail ?></a>: la richiesta viene evasa come ogni altra istanza ai sensi degli artt. 15/17/20 GDPR.</p>
...
<h2>11. Minors</h2>
<p>A public Facebook Page has no way to verify the age of who comments: the Platform does not request or receive any age signal from users. The presence of minors among commenters is therefore <strong>foreseeable</strong>, not an exception the Controller can rule out by policy. For the same reason, the Controller cannot promise to recognise and "immediately" delete a minor's data at the point of collection: no reliable technical mechanism exists, or could exist, to infer age from a public comment.</p>
<p>The response to this foreseeability is not denial, but the same safeguards already applied to <em>every</em> user from their first comment, regardless of age: no ban on the first violation (a recidivism pattern is required, see §5.4), every ban is always temporary, human review of uncertain cases is blind (the moderator never sees the real name, see §5.4), and every decision — automated or human — remains contestable via the appeal procedure (§8).</p>
<p>A minor, or their legal guardian, may still request deletion of their data or exercise any of the rights described in §8 at any time by writing to <a href="mailto:<?= $orgEmail ?>"><?= $orgEmail ?></a>: the request is handled like any other request under Arts. 15/17/20 GDPR.</p>
