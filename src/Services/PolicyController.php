 ...
 * ─────────────────────────
 * Each policy record stores TWO prompt fields:
 *
 ...
 *
 *   system_prompt      — deprecated legacy field kept for backward compatibility.
 *                        New policies must use moderation_prompt; system_prompt is
...
class PolicyController
{
    ...
    public function create(ServerRequestInterface $request, Response $response): ResponseInterface
    {
        ...
        // Accept either the new moderation_prompt or the legacy system_prompt
        $moderationPrompt = trim($body['moderation_prompt'] ?? $body['system_prompt'] ?? '');
        if ($moderationPrompt === '') {
        ...
        $id = DB::table('policies')->insertGetId([
            ...
            'system_prompt'     => $moderationPrompt, // keep legacy column in sync
    ...
    public function update(ServerRequestInterface $request, Response $response, array $args): ResponseInterface
    {
        ...
        $currentPrompt = $policy->moderation_prompt ?? $policy->system_prompt ?? '';

        // Prompt changes require a new version
        $newPrompt = $body['moderation_prompt'] ?? $body['system_prompt'] ?? null;
